...
html:
  ...
  - name: y0
    type: number
    bs3caption: "Conductor sag 1, feet"
    value: 5.75
  - name: y0b
    type: number
    bs3caption: "Conductor sag 2, feet"
    value: 5.75
  - name: x1
    type: number
...
```js
          //: output=markdown
idx = _.map(d.conductors, String).indexOf(conductor1)
wc = d.wc[idx]
area = d.area[idx]
idx = _.map(d.conductors, String).indexOf(conductor2)
wcb = d.wc[idx]
areab = d.area[idx]

t = ficm(flti, fltt, h0, y0, y0b, x1, y1, wc, area, wcb, areab, 0)
wirelocs = ficm(flti, fltt, h0, y0, y0b, x1, y1, wc, area, wcb, areab, 1)
...
plot = $.plot($('#graph'),
              ...
              { colors: ["#ecb204", "rgba(#087fdd, 0.83)"],
                ...
                yaxis: { min: Math.min(-2, y1 - y0b), max: Math.max(2*y0 + x1) }});
...
function bisect(flti) {
    ...
    do {
        delta = (hi - lo)/2.0
        t = ficm(flti, lo + delta, h0, y0, y0b, x1, y1, wc, area, wcb, areab, 0)
